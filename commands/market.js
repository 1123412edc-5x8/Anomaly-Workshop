...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        // 3. 賣出 (修正順序：Required 必須在前面)
        .addSubcommand(sub => 
            sub.setName('sell')
                .setDescription('售出物資')
                .addStringOption(o => o.setName('item').setDescription('選擇物品').setRequired(true).setAutocomplete(true))
                .addBooleanOption(o => o.setName('sell_all').setDescription('是否全部出售？').setRequired(true))
                .addIntegerOption(o => o.setName('amount').setDescription('出售數量 (若選全部則此項失效)').setMinValue(1).setRequired(true))), // 統一改為必填防止報錯
    ...
    async execute(interaction) {
        ...
        if (!player) return interaction.reply({ content: '❌ 找不到您的資料。', ephemeral: true });
        ...
        if (sub === 'buy') {
            ...
            db.write(data);
            return interaction.reply(`✅ 購買成功！${itemName} 漲至 \`${data.market_prices[itemName]}\``);
        }
        ...
        if (sub === 'sell') {
            ...
            const sellCount = sellAll ? indices.length : Math.min(amountInput, indices.length);
            ...
            db.write(data);
            return interaction.reply(`💰 賣出成功！${itemName} 跌至 \`${data.market_prices[itemName]}\``);
        }
    }
};